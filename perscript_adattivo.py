...
def gradient(X , numbers_gd_loops, step_width = 0.2, max_volume_box = 0.25, epsilon = 0.01, R2_alg_orig = False,  ZPLtau_alg_orig = True):
    # np.random.seed(10)
    out = {}
    ...
    for gd in range(numbers_gd_loops):
        ...
        while any(differenza > 0.8):
        #    i = i + 1
        #    print("loop interno", i)
...
if __name__ == "__main__":
    df = pd.read_csv(r'Generated_data/dati_generati_originali_5perc.csv')
    ...
    prova = gradient(numbers_gd_loops = 200, X = X_df_trasf, R2_alg_orig = False, ZPLtau_alg_orig = True)
